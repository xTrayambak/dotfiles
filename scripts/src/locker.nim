...
proc lockUp() {.inline.} =
  ...
  let wallpaper =
    getHomeDir() / "Wallpapers" / "courtney-kenady-h5ZPU1hU5CM-unsplash.jpg"
    # getWallpaper("")
  info "Chosen wallpaper: " & wallpaper

  copyFile(wallpaper, "/tmp/current-wallpaper")
  ...
  # setWallpaperState(WallpaperState(paused: true))

  info "Forcing volume to 0%"
  discard execCmd("volumectl mute")
    # for the silly apps that play sounds even after pausing players (like games)

  info "Executing lock"
  discard execCmd("hyprlock")
